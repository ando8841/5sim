...
const PriceList = ({ country, countriesNames, service, priceFilter }) => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = usePricesByCountryAndProduct(country, service);
    ...
    <>
      ...
      <List>
        {filteredPrices?.length ? (
          filteredPrices.map(
            ({ key, country: priceCountry, count, rate, cost, name }) => (
              <ListItem
                key={key}
                sx={{ border: '1px solid #7b97a8', borderRadius: 2, margin: 1 }}
              >
                <ListItemText
                  primary={
                    country
                      ? countriesNames[country]
                      : countriesNames[priceCountry]
                  }
                  secondary={
                    <>
                      {name}{' '}
